use swc_core::{
  common::DUMMY_SP,
  ecma::{
    ast::{Expr, Id, Ident, JSXElement, JSXElementChild, JSXElementName, JSXExpr, JSXExprContainer, BinExpr, BinaryOp, AssignExpr, AssignOp, PatOrExpr, JSXAttr, JSXAttrName},
...
pub fn modify_jsx_if_immutable(jsx_ele: &mut JSXElement, state: &mut State) -> bool {
  ...
  let mut self_immutable = match &jsx_ele.opening.name {
    JSXElementName::Ident(ident) => {
      ident.sym.chars().next().unwrap().is_lowercase() || state.vars.contains(&ident.to_id())
    }
  ...
  if self_immutable {
    ...
    jsx_ele.opening.attrs.visit_with(&mut immutable_pass);
...
pub fn are_children_immutable(children: &mut [JSXElementChild], state: &mut State, allow_hoist_child: bool) -> bool {
...
struct Immutable<'a> {
  vars: &'a rustc_hash::FxHashSet<Id>,
  immutable: bool,
}

impl<'a> Visit for Immutable<'a> {
  noop_visit_type!();
  ...
  fn visit_expr(&mut self, expr: &Expr) {
    ...
    match expr {
      Expr::Ident(id) => {
        if !self.vars.contains(&id.to_id()) {
          self.immutable = false
        }
      },
      Expr::Lit(_) => {},
